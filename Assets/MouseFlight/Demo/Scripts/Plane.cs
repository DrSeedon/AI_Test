...
namespace MFlight.Demo
{
    ...
    {
        [Header("Components")]
        [SerializeField] private MouseFlightController mouseController = null;
        [SerializeField] private HandFlightController handFlightController = null;
        [SerializeField] private CinemachineVirtualCamera followCam = null;
        ...
        [Header("Autopilot")] 
        public bool isActiveAutopilot = true;
        ...
        private void Awake()
        {
            ...
            if (handFlightController == null)
                Debug.LogError(name + ": Plane - Missing reference to HandFlightController!");
        }
        ...
        private void Update()
        {
            ...
            if (Mathf.Abs(keyboardRoll) > .01f)
            {
                rollOverride = true;
            }

            float keyboardPitch = Input.GetAxis("Vertical");
            if (Mathf.Abs(keyboardPitch) > .01f)
            {
                pitchOverride = true;
                rollOverride = true;
            }
            ...
            float autoRoll = 0f;
            if (mouseController != null)
                if (isActiveAutopilot)
                {
                    RunAutopilot(mouseController.MouseAimPos, out autoYaw, out autoPitch, out autoRoll);
                    followCam.Follow = mouseController.cameraRig;
                    followCam.LookAt = mouseController.cameraRig;
                }
            if (handFlightController != null)
                if (!isActiveAutopilot)
                {
                    pitch = handFlightController.PitchValue;
                    roll = handFlightController.RollValue;
                }
            ...
            if (isKeyboardControl)
            {
                ...
                yaw = (yawOverride) ? keyboardYaw : autoRoll;
            }

            mouseController.LockCamera(isLockCamera);
