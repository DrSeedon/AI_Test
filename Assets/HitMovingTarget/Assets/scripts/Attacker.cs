...
public class Attacker : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        //velocityTarget = (target.position - oldPositionTarget) / Time.fixedDeltaTime; //запоминаем скорость;
        //oldPositionTarget = target.position; //берем новую "старую" позицию
    }
    ...
    IEnumerator CheckSpeed()
    {
        while (true)
        {
            velocityTarget = (target.position - oldPositionTarget) / 0.1f; //запоминаем скорость;
            oldPositionTarget = target.position; //берем новую "старую" позицию
    ...
    private void Shot()
    {
        ...
        float time = 0;
        Vector3 hitPoint = GetHitPoint(target.position, velocityTarget, transform.position, bulletSpeed, out time);
